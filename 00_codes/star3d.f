      ...
      real*8 area,aringsi,areap,areapn
...
C     The variable aringsi is the area in SI units of the strip of length 
C     dr1r2 (height of a cell measured along the meridian) and length
C     the circumference of radius as  measured from the rotation axis.
C     
C     r and dr1r2 are in units of the equatorial radius, so to convert the
C     whole expression into SI units we have to multiply by req_si**2

         aringsi=2.0d0*pi*r*cth*dr1r2*req_si**2
...
C     Computing an average value of T and log g (step 1)
 ...
 6      continue
        ...
        write(10,1004) ith,t_k,lg,m1,m2,m3,m4,c1,c2,aringsi
        model(k)=m1
        model(k+1)=m2
        model(k+2)=m3
        model(k+3)=m4
...
C        write(11,1004) ith,t_k_vz,lg,m1,m2,m3,m4,c1,c2
 ...
 1004   format(1x,f6.2,1x,f7.1,1x,f5.3,4(1x,a10),1x,
