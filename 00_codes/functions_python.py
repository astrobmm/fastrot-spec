...
def create_input_grid_file(deltaphi, deltatheta)

    """
    deltaphi:   tile separation in longitude phi [deg]
    deltatheta: tile separation in latitude  theta [deg]
    """
    ...
    with open(path_input_file, 'w') as file:
        ...
        file.write(f'{deltaphi}' + '\n')
        file.write(f'{deltatheta}' + '\n')

def run_fastrot_highres-spec(i = 5.0, ms = 2.15, t = 10000, rs = 2.726, o = 0.632, met = '-0.5'):
    create_input_star_file(i, ms, t, rs, o, met)
    create_input_grid_file(deltaphi, deltatheta)
    os.system('./fastrot-highres-spec.sh')

def run_fastrot_lowres-spec(i = 5.0, ms = 2.15, t = 10000, rs = 2.726, o = 0.632, met = '-0.5'):
    create_input_star_file(i, ms, t, rs, o, met)
    create_input_grid_file(deltaphi, deltatheta)
